...
export default class SolarSystem {
		...
		try {
			...
			const label = MRE.Actor.Create(this.context, {
				actor: {
					...
					transform: {
						local: { position: { x: 2.2, y: 0.3, z: 0 } }
					}
				}
			});
			...
			buttonBehavior.onHover('exit', () => {
				...
				label.text.contents = 'Hoverd Out';
			});

			

			buttonBehavior.onClick(() => {
				this.createScaleAnimations(bodyName);
				label.text.contents = 'You clicked';
				//model.transform.local.scale.x = 0.2345;
				//model.transform.local.scale.y = 0.2345;
				//model.transform.local.scale.z = 0.2345;
	...
	private createScaleAnimations(bodyName: string) {
		const facts = database[bodyName];
